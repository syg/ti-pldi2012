...
\item The three benchmark suites described in Section~\ref{XXX}.
...
whereas \code{undefined}, \code{null} and booleans are either zero or one.
...
The inferred precision section of Figure~\ref{XXX} shows the fractions
...
The arithmetic and index sections of Figure~\ref{XXX} show the frequency
...
distinction drawn in Section~\ref{XXX}.
Figure~\ref{XXX} shows the fractions of indexed accesses on arrays
...
of operations known to be on packed arrays (Section~\ref{XXX}),
...
In the property access section of Figure~\ref{XXX},
the definite column shows the fraction
of operations which were statically resolved as definite properties
(Section~\ref{XXX}), while the PIC column shows the fraction which
...
Some of the numbers from these runs are shown in Figure~\ref{XXX}.
