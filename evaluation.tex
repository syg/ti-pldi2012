...
Measurements for each of these websites are on a workload where
the site was used for several minutes, exercising various features.

\item The membench50 suite \cite{XXX}, a memory testing framework
which loads the front pages of 50 popular websites.

\item The three benchmark suites covered in Section~\ref{XXX}.

\item Six games and demos which are bound on Javascript performance.
These were used for several minutes or, in the case of non-interactive
demos, viewed to completion.

\end{itemize}


INTRO % describe sample pages, contrast websites and games/demos
