...
\left\{
\begin{array}{l}
o \in T_x \Implies T_{x.p} \barrier \mathit{prop}(o,p)\ |\ \text{all } o
\end{array}
\right\} \tag{\sc Prop}\\
& x[i] &&
\left\{
\begin{array}{l}
o \in T_x \Implies T_{x[i]} \barrier \mathit{index}(o)\ |\ \text{all } o
\end{array}
\right\} \tag{\sc Index}\\
...
\left\{
\begin{array}{l}
 o \in T_x \Implies \mathit{prop}(o,p) \supseteq T_e\ |\ \text{all } o
\end{array}
\right\} \tag{\sc A-Prop}\\
& x[i] = e &&
\left\{
\begin{array}{l}
o \in T_x \Implies \mathit{index}(o) \supseteq T_e\ |\ \text{all } o
\end{array}
\right\} \tag{\sc A-Index}\\
...
$C$, $A \Implies C$ takes on the meaning of $C$ if $A$ holds during
propagation and is otherwise ignored.
...
barrier constraint on line 8 is $T_{\code{a[i]}} \barrier \mathit{prop}(\code{Box}, \code{p})$,
with $T_{\code{a[i]}} = \{\code{int}\}$ and $\mathit{prop}(\code{Box}, \code{p}) = \{\code{int},
\code{string}\}$. Since $T_{\code{a[i]}} \not\supseteq \mathit{prop}(\code{Box}, \code{p})$, a type
barrier is required.

In the constraint generation rules in Figure~\ref{fig:constraint-rules} we
present two rules which employ type barriers:
