...
C & ::= T \supseteq T\ |\ T \barrier T\ |\ A \Implies T && \text{constraints}
\end{align*}
...
standard subset constraint, $\supseteq$, a \emph{barrier subset
  constraint}, styled $\barrier$, and the conditional constraint, $\Implies$.
...
$C$, $A \Implies C$ means that if $A$ holds during propagation, resolve the
constraint $C$, otherwise do nothing.
