...
the core language is \code{if}, which tests for definedness. The core contains
...
Constraint generation and propagation can happen both during analysis time and
...
lines and provide the desugaring where needed. In the following walkthrough we will show how the variable \code{v} on line 8 gets the type \code{int}.
...
$T_{\code{au}}$. And at the current time, before \code{use} is actually
...
the dynamic check we inserted at the argument binding for \code{use} is
...
interesting bits happen when \code{use} is actually executed. When execution
...
\code{int} from $\mathit{prop}(A, \code{p})$ to $T_{\code{v}}$.

At this point, as we have observed all possible types of the property access \code{a[i].p}, we can remove the emitted dynamic check. But this removal may not be permanent. If we analyze new code which adds new types to the type set $\mathit{prop}(\mathit{Box}, \code{p})$, we will need to re-emit the dynamic check. For instance, if there were a use of \code{Box} elsewhere in the code which constructs it with a \code{string} argument, we would not be able to remove the dynamic check.
