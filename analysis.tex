...
In many cases type information is insufficient by itself to generate code
for an operation which runs at a speed comparable to a statically typed
...
To reach a speed closer to a statically typed language, these checks must
be eliminated in as many cases as possible,
and doing so requires more detailed analysis information.
...
and we have measured overflow check overhead at 10-20\% of total execution
time on microbenchmarks.

Using type information, we normally know statically where integers are being
added, and use two techniques to remove overflow checks.
...
and mark the type of the array as possibly not packed.
...
middle of the array, and only needs to perform a bounds check (which can
often be hoisted from loop bodies, in the same manner as integer overflow
checks).

The packed status of an object type can change dynamically due to out of
order writes, possibly invalidating JIT code.
This forces recompilation in the same way as type changes due to semantic
triggers and type barriers being tripped.
...
Internally, objects are laid out as an array of values with the map associating
each property name to a slot in that array.
...
as a direct lookup comparable to
field accesses in a language with structural typing, such as Java or C++.
...
adds properties.
Third, objects created by calling 'new' on the same function will have the
same prototype (unless the function's \code{prototype} property is overwritten),
and we analyze the function's script to identify properties it definitely
...
in the same way as packed array or type set changes.
